...
use crate::{
    downloader::DiscoveredMusicTrack,
    utils::filename::parse_track_metadata,
};

fn youtube_meta_to_discovered(meta: &yt_audio_downloader::models::VideoMetadata) -> DiscoveredMusicTrack {
    let id = yt_audio_downloader::youtube_id_to_i64(&meta.id);
    let permalink_url =
        Url::parse(&format!("https://www.youtube.com/watch?v={}", meta.id)).ok();
    let artwork_url = meta.thumbnail_url.as_deref().and_then(|u| Url::parse(u).ok());
...
pub async fn discover_youtube_tracks(url: &Url) -> Result<Vec<DiscoveredMusicTrack>> {
    ...
    if let Some(playlist_id) = extract_playlist_id(url_str) {
        ...
        Ok(metadata_list.iter().map(youtube_meta_to_discovered).collect())
    } else {
        let downloader = YoutubeAudioDownloader::new();
        let meta = downloader
